...
def processar(acomp, ger, fusao):
    ...
    cols_ger_ref = [c for c in [
        ...
        'material_fusao','material_comercial','semana_fusao','data_fusao'
    ...
    for _, r in tabela.iterrows():
        try:
            ...
            acomp_records.append({
                ...
                # TT especial: se QUALQUER TT tem prev/real = aplicável
                'tt': 'OK' if any(pd.notna(r.get(f)) for f in ['tempera_real','revenimento_real','normalizacao_corte_real','alivio_tensoes_real']) 
                      else ('!' if any(pd.notna(r.get(f)) for f in ['tempera_prev','revenimento_prev','normalizacao_corte_prev','alivio_tensoes_prev'])
                      else '-'),
