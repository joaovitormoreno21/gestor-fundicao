...
def normalizar_texto_chave(v):
    """Normaliza texto para cruzamentos: maiúsculo, sem espaços duplicados."""
    if pd.isna(v):
        return ''
...
def processar(acomp, ger, fusao):
    ...
    if not receitas.empty:
        ...
        # Liga usada para cruzamento com a receita: material_fusao_corrigida, com fallback para material_fusao.
        ...
        if not mp_base.empty:
            ...
            # Relatório por semana + matéria prima + liga.
            resumo_mp = (
                mp_base.groupby(['ano','semana','liga','cod_materia_prima','desc_materia_prima'], dropna=False)
                .agg(
                    qtd_kg=('qtd_materia_prima_kg','sum'),
                    metal_planejado_kg=('metal_liq_calc','sum'),
                    itens=('chave','nunique')
                )
                .reset_index()
                .sort_values(['semana','desc_materia_prima','liga'])
            )

            for _, r in resumo_mp.iterrows():
                try:
                    mp_records.append({
                        ...
                        'qtd_kg': round(numero(r.get('qtd_kg'), 0), 0),
                        'qtd_t': round(numero(r.get('qtd_kg'), 0) / 1000, 2),
                        'metal_planejado_t': round(numero(r.get('metal_planejado_kg'), 0) / 1000, 2),
                        'itens': int(r.get('itens')) if pd.notna(r.get('itens')) else 0,
                    })
                except Exception:
                    pass

    log(f'  ✓ Matéria Prima calculada: {len(mp_records)} linhas de relatório')
  ...
  <div class="tab-content" id="tab-materia">
    ...
    <div class="card">
      ...
      <div class="twrap">
        <table>
          <thead><tr>
            <th onclick="srtMp('semana')">Semana</th>
            <th onclick="srtMp('liga')">Liga</th>
            <th onclick="srtMp('cod')">Código</th>
            <th onclick="srtMp('descricao')">Descrição matéria-prima</th>
            <th onclick="srtMp('qtd_kg')">Qtd. kg</th>
            <th onclick="srtMp('qtd_t')">Qtd. t</th>
            <th onclick="srtMp('itens')">Itens</th>
          </tr></thead>
...
let sk='os', sd=1, p=1, pAcomp=1, pMp=1, skA='os', sdA=1, skMp='semana', sdMp=1;
...
function applyFMp(){{
  ...
  mpFil=MP.filter(r=>{{
    ...
    if(q&&!(`${{r.semana}} ${{r.liga}} ${{r.cod}} ${{r.descricao}}`).toLowerCase().includes(q))return false;
    return true;
  }});
  pMp=1;
  renderKMp();
...
function renderKMp(){{
  ...
  const itens=mpFil.reduce((s,r)=>s+(Number(r.itens)||0),0);
  ...
  document.getElementById('mp-label').textContent=`${{mpFil.length}} linhas`;
}}
function srtMp(k){{if(skMp===k)sdMp*=-1;else{{skMp=k;sdMp=1;}}renderTMp();}}
function renderTMp(){{
  const sorted=[...mpFil].sort((a,b)=>{{
    let va=a[skMp],vb=b[skMp];
    if(va==null)return 1;
    if(vb==null)return-1;
    return typeof va==='string'?va.localeCompare(vb)*sdMp:(va-vb)*sdMp;
  }});
  const sl=sorted.slice((pMp-1)*PP,pMp*PP);
  document.getElementById('tbody-mp').innerHTML=sl.map(r=>`<tr>
    <td style="text-align:center;font-weight:600;">${{r.semana||'-'}}</td>
    <td>${{r.liga}}</td>
    <td style="color:#5f5e5a;">${{r.cod}}</td>
    <td style="max-width:360px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${{r.descricao}}">${{r.descricao}}</td>
    <td style="text-align:right;">${{Math.round(Number(r.qtd_kg)||0).toLocaleString('pt-BR')}}</td>
    <td style="text-align:right;">${{Math.round(Number(r.qtd_t)||0).toLocaleString('pt-BR')}}</td>
    <td style="text-align:right;">${{(Number(r.itens)||0).toLocaleString('pt-BR')}}</td>
  </tr>`).join('');
  const pages=Math.max(1,Math.ceil(mpFil.length/PP));
  document.getElementById('tcnt-mp').textContent=mpFil.length;
