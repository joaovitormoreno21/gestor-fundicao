...
function renderMpGrupoResumo(){{
  ...
  el.innerHTML=rows.map(r=>`<div class="mp-summary-item">${{r.grupo?('G'+String(r.grupo).replace(/^G/i,'')):'G?'}} - ${{r.desc}} = <span class="mp-summary-value">${{Math.round(r.metal).toLocaleString('pt-BR')}}</span></div>`).join('');
}}
