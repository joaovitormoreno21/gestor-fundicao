...
def localizar_receitas_padrao():
    """Procura o arquivo Receitas Padrão.xlsx na pasta do projeto e na pasta dados."""
    candidatos = [
        BASE_DIR / 'Receitas Padrão.xlsx',
        BASE_DIR / 'Receitas Padrao.xlsx',
        BASE_DIR / 'dados' / 'Receitas Padrão.xlsx',
        BASE_DIR / 'dados' / 'Receitas Padrao.xlsx',
    ]
...
def normalizar_texto_chave(v):
    """
    Normaliza texto para cruzamentos de liga/material.
    Ignora maiúsculas/minúsculas, espaços, traços, barras, pontos e outros símbolos.
    Ex.: AM-XT710, AMXT710, am xt710 -> AMXT710
    """
    import re
    if pd.isna(v):
        return ''
...
def carregar_receitas_padrao():
    """
    Lê o arquivo Receitas Padrão.xlsx.
    Estrutura esperada:
      Liga | Cod | Descrição | Kg/Tonelada | %carga

    Retorna DataFrame com:
      liga, cod_materia_prima, desc_materia_prima, kg_por_tonelada, perc_carga
    """
    arq = localizar_receitas_padrao()
    if arq is None:
        log('  ⚠ Receitas Padrão.xlsx não encontrado. Aba Matéria Prima ficará vazia.')
        log('    Coloque o arquivo na pasta do projeto ou em dados/Receitas Padrão.xlsx')
        return pd.DataFrame(columns=[
            'liga','liga_key','cod_materia_prima','desc_materia_prima',
            'kg_por_tonelada','perc_carga'
        ])

    try:
        receitas = pd.read_excel(arq)
        receitas.columns = [str(c).strip() for c in receitas.columns]

        # Aceita nomes próximos, mas prioriza os nomes do arquivo enviado.
        col_liga = next((c for c in receitas.columns if c.lower() in ['liga', 'ligas']), receitas.columns[0])
        col_cod = next((c for c in receitas.columns if c.lower() in ['cod', 'código', 'codigo']), receitas.columns[1])
        col_desc = next((c for c in receitas.columns if 'descr' in c.lower()), receitas.columns[2])
        col_kg = next((c for c in receitas.columns if 'kg' in c.lower() and ('ton' in c.lower() or 'tonelada' in c.lower())), receitas.columns[3])
        col_perc = next((c for c in receitas.columns if '%' in c or 'carga' in c.lower()), receitas.columns[4])

        receitas = receitas[[col_liga, col_cod, col_desc, col_kg, col_perc]].copy()
        receitas.columns = ['liga','cod_materia_prima','desc_materia_prima','kg_por_tonelada','perc_carga']

        receitas['liga'] = receitas['liga'].astype(str).str.strip()
        receitas['liga_key'] = receitas['liga'].apply(normalizar_texto_chave)
        receitas['desc_materia_prima'] = receitas['desc_materia_prima'].astype(str).str.strip()
        receitas['kg_por_tonelada'] = pd.to_numeric(receitas['kg_por_tonelada'], errors='coerce').fillna(0)
        receitas['perc_carga'] = pd.to_numeric(receitas['perc_carga'], errors='coerce').fillna(0)

        receitas = receitas[
            (receitas['liga_key'] != '') &
            (receitas['desc_materia_prima'] != '') &
            (receitas['desc_materia_prima'].str.lower() != 'nan')
        ].copy()

        log(f'  ✓ Receitas Padrão carregada: {len(receitas)} linhas ({receitas["liga"].nunique()} ligas)')
        return receitas
    except Exception as e:
        log(f'  ⚠ Não consegui ler Receitas Padrão.xlsx: {e}')
        return pd.DataFrame(columns=[
            'liga','liga_key','cod_materia_prima','desc_materia_prima',
            'kg_por_tonelada','perc_carga'
        ])
...
def processar(acomp, ger, fusao):
    ...
    for _, r in ger_proc.iterrows():
        try:
            ...
            ger_records.append({
                ...
                'codigo': str(r.get('codigo_peca') or ''),
                'sequencia': str(r.get('seq_peca') or ''),
                ...
                'liga_planejada': str(r.get('material_fusao_corrigida') or r.get('material_fusao') or r.get('material_comercial') or '').strip(),
    ...
    # -------------------- Matéria Prima / necessidade planejada --------------------
    receitas = carregar_receitas_padrao()
    receitas_records = []
    if not receitas.empty:
        for _, rr in receitas.iterrows():
            try:
                receitas_records.append({
                    'liga': str(rr.get('liga') or ''),
                    'liga_key': str(rr.get('liga_key') or ''),
                    'cod': str(rr.get('cod_materia_prima') or ''),
                    'descricao': str(rr.get('desc_materia_prima') or ''),
                    'kg_por_tonelada': round(numero(rr.get('kg_por_tonelada'), 0), 3),
                    'perc_carga': round(numero(rr.get('perc_carga'), 0), 3),
                })
            except Exception:
                pass

    # mp_records agora é detalhado, uma linha por peça planejada x matéria-prima da receita.
    # O HTML monta a matriz por descrição de matéria-prima, semanas e total.
    mp_records = []
    mp_alertas = []

    if not receitas.empty:
        # Base: somente itens planejados, ou seja, sem data_fusao.
        planejados_mp = ger_proc[pd.isna(ger_proc.get('data_fusao'))].copy()

        if 'semana_fusao' in planejados_mp.columns:
            planejados_mp = planejados_mp[planejados_mp['semana_fusao'].notna()].copy()

        # Liga usada para cruzamento com a receita: material_fusao_corrigida, com fallback para material_fusao/material_comercial.
        planejados_mp['liga_receita'] = planejados_mp.apply(
            lambda r: str(r.get('material_fusao_corrigida') or r.get('material_fusao') or r.get('material_comercial') or '').strip(),
            axis=1
        )
        # Chave ignora traços/espaços/símbolos: AM-XT710 = AMXT710.
        planejados_mp['liga_key'] = planejados_mp['liga_receita'].apply(normalizar_texto_chave)

        ligas_receita = set(receitas['liga_key'].dropna().unique())
        ligas_planejadas = set(planejados_mp['liga_key'].dropna().unique())
        ligas_sem_receita = sorted([l for l in ligas_planejadas if l and l not in ligas_receita])

        if ligas_sem_receita:
            log('\n⚠ LIGAS PLANEJADAS SEM RECEITA PADRÃO:')
            liga_display = (
                planejados_mp[['liga_key','liga_receita']]
                .dropna()
                .drop_duplicates('liga_key')
                .set_index('liga_key')['liga_receita']
                .to_dict()
            )
            for liga in ligas_sem_receita[:50]:
                nome_liga = liga_display.get(liga, liga)
                log(f'  - {nome_liga}')
                mp_alertas.append(nome_liga)
            if len(ligas_sem_receita) > 50:
                log(f'  ... +{len(ligas_sem_receita)-50} ligas')
            log('Cadastre essas ligas no arquivo Receitas Padrão.xlsx para entrar no cálculo.\n')

        # Junta cada peça planejada com as linhas de receita da respectiva liga.
        cols_plan = [
            'id_ordem_servico','item','chave','cliente','nome_fantasia','descricao',
            'semana_fusao','liga_receita','liga_key','metal_liq_calc','Grupo','Desc_Grupo'
        ]
        cols_plan = [c for c in cols_plan if c in planejados_mp.columns]
        mp_base = planejados_mp[cols_plan].merge(receitas, on='liga_key', how='inner')

        if not mp_base.empty:
            mp_base['semana'] = pd.to_numeric(mp_base['semana_fusao'], errors='coerce').fillna(0).astype(int)
            mp_base['ano'] = mp_base['semana'].astype(str).str[:4]
            mp_base['metal_liq_calc'] = pd.to_numeric(mp_base['metal_liq_calc'], errors='coerce').fillna(0)
            mp_base['kg_por_tonelada'] = pd.to_numeric(mp_base['kg_por_tonelada'], errors='coerce').fillna(0)
            mp_base['qtd_materia_prima_kg'] = (mp_base['metal_liq_calc'] / 1000) * mp_base['kg_por_tonelada']

            for _, r in mp_base.iterrows():
                try:
                    qtd_kg = numero(r.get('qtd_materia_prima_kg'), 0)
                    semana = int(r.get('semana')) if pd.notna(r.get('semana')) else 0
                    os_val = r.get('id_ordem_servico')
                    item_val = r.get('item')
                    chave_val = str(r.get('chave') or f"{os_val or ''}-{item_val or ''}")
                    mp_records.append({
                        'ano': str(r.get('ano') or ''),
                        'semana': semana,
                        'liga': str(r.get('liga') or r.get('liga_receita') or ''),
                        'liga_planejada': str(r.get('liga_receita') or ''),
                        'grupo': str(r.get('Grupo') or ''),
                        'desc_grupo': str(r.get('Desc_Grupo') or 'Sem grupo'),
                        'cod': str(r.get('cod_materia_prima') or ''),
                        'descricao': str(r.get('desc_materia_prima') or ''),
                        'qtd_kg': round(qtd_kg, 0),
                        'qtd_t': round(qtd_kg / 1000, 3),
                        'kg_por_tonelada': round(numero(r.get('kg_por_tonelada'), 0), 3),
                        'perc_carga': round(numero(r.get('perc_carga'), 0), 3),
                        'metal_planejado_kg': round(numero(r.get('metal_liq_calc'), 0), 0),
                        'metal_planejado_t': round(numero(r.get('metal_liq_calc'), 0) / 1000, 3),
                        'os': int(os_val) if pd.notna(os_val) else '',
                        'item': int(item_val) if pd.notna(item_val) else '',
                        'chave': chave_val,
                        'cliente': short(r.get('nome_fantasia') or r.get('cliente') or '', 40),
                        'peca': short(r.get('descricao') or '', 65),
                    })
                except Exception:
                    pass

    log(f'  ✓ Matéria Prima calculada: {len(mp_records)} linhas detalhadas')
...
def salvar_excel(acomp_original, ger_processado, fusao, receitas_padrao=None):
    ...
    try:
        ...
        receitas_excel = limpar_para_excel(receitas_padrao) if receitas_padrao is not None and not receitas_padrao.empty else None
        apoio_excel = None
        if EXCEL_PATH.exists():
            try:
        ...
        with pd.ExcelWriter(EXCEL_PATH, engine='openpyxl') as w:
            ...
            if receitas_excel is not None:
                receitas_excel.to_excel(w, sheet_name='Receitas padrão', index=False)
...
.mp-summary-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:0;border:1px solid rgba(0,0,0,.14);border-radius:8px;overflow:hidden;background:#fff}}
.mp-summary-item{{padding:8px 10px;border-right:1px solid rgba(0,0,0,.14);border-bottom:1px solid rgba(0,0,0,.14);font-size:12px;font-weight:600;color:#1a1a18;background:#f8f7f4}}
.mp-summary-item:nth-child(3n){{border-right:none}}
.mp-summary-value{{font-weight:700;color:#185fa5}}
.mp-link{{color:#185fa5;font-weight:600;cursor:pointer;text-decoration:underline;text-underline-offset:2px}}
.sim-box{{background:#f8f7f4;border:1px solid rgba(0,0,0,.12);border-radius:10px;padding:10px;margin-bottom:10px}}
.sim-form{{display:grid;grid-template-columns:2fr 1fr 1fr auto auto;gap:8px;align-items:end}}
.sim-form label{{font-size:10px;color:#5f5e5a;text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:4px}}
.sim-form select,.sim-form input{{width:100%;border:0.5px solid rgba(0,0,0,.22);border-radius:8px;background:#fff;color:#1a1a18;padding:7px 10px;font-size:12px;outline:none}}
.sim-mini{{font-size:11px;color:#888780;margin-top:6px}}
@media(max-width:900px){{
  ...
  .mp-summary-item{{border-right:none}}
...
<div class="container">
  <div class="tabs">
    ...
    <button class="tab" onclick="switchTab('materia')">Matéria Prima</button>
    <button class="tab" onclick="switchTab('simmp')">Simulação MP</button>
  </div>
  ...
  <div class="card">
    ...
    <div class="twrap">
      <table>
        <thead><tr>
          <th onclick="srt('os')">OS</th><th onclick="srt('item')">Item</th><th onclick="srt('codigo')">Código</th><th onclick="srt('sequencia')">Seq</th><th onclick="srt('cliente')">Cliente</th>
          <th onclick="srt('descricao')">Descrição</th><th onclick="srt('material')">Material</th><th onclick="srt('semana')">Sem. Fusão</th>
          <th onclick="srt('prz_fund')">Prz. Fund.</th><th onclick="srt('prz_cont')">Prz. Cont.</th><th onclick="srt('etapa')">Etapa Atual</th><th>Obs</th>
        </tr></thead>
  ...
  <div class="tab-content" id="tab-acomp">
    <div class="kpi-grid">
      <div class="kpi ok"><div class="klabel">Itens no prazo</div><div class="kval" id="k-acomp-prazo">—</div></div>
      <div class="kpi warn"><div class="klabel">Potencial atraso</div><div class="kval" id="k-acomp-potencial">—</div></div>
      <div class="kpi" style="border-color:rgba(226,75,74,.35)"><div class="klabel">Atrasado</div><div class="kval" id="k-acomp-atrasado" style="color:#e24b4a">—</div></div>
    </div>
    ...
    <div class="card">
      ...
      <div class="twrap">
        <table>
          <thead><tr>
            <th onclick="srtA('codigo')">Código</th><th onclick="srtA('sequencia')">Seq</th><th onclick="srtA('cliente')">Cliente</th>
            ...
            <th>Acab.</th><th>Usin.</th><th>Lib.<br>Fund.</th><th>Obs</th>
          </tr></thead>
  ...
  <!-- ABA MATÉRIA PRIMA -->
  <div class="tab-content" id="tab-materia">
    <div class="kpi-grid">
      <div class="kpi info"><div class="klabel">Matéria-prima total</div><div class="kval" id="k-mp-total">—</div></div>
      <div class="kpi warn"><div class="klabel">Ligas com receita</div><div class="kval" id="k-mp-ligas">—</div></div>
      <div class="kpi ok"><div class="klabel">Itens planejados vinculados</div><div class="kval" id="k-mp-itens">—</div></div>
    </div>

    <div class="filters">
      <span style="font-size:11px;color:#5f5e5a;"><i class="ti ti-filter" style="font-size:13px;vertical-align:-2px;" aria-hidden="true"></i> Filtros Matéria Prima:</span>
      <select id="f-mp-ano"><option value="">Todos os anos</option></select>
      <select id="f-mp-semana"><option value="">Todas as semanas</option></select>
      <select id="f-mp-liga"><option value="">Todas as ligas</option></select>
      <select id="f-mp-desc"><option value="">Todas matérias-primas</option></select>
      <input id="f-search-mp" type="text" placeholder="Buscar código, descrição ou liga..." style="min-width:220px;">
      <button class="btn" onclick="resetFMp()"><i class="ti ti-x" style="font-size:11px;"></i> Limpar</button>
      <span style="margin-left:auto;font-size:11px;color:#888780;" id="mp-label"></span>
    </div>

    <div class="card" style="margin-bottom:10px;display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap;">
      <div>
        <div class="ctitle" style="margin-bottom:3px;">Compra de matéria-prima</div>
        <div style="font-size:12px;color:#5f5e5a;">Use a simulação para incluir semanas planejadas, novas ligas e comparar consumo.</div>
      </div>
      <button class="btn" onclick="goSimulacao()" style="background:#185fa5;color:#fff;border-color:#185fa5;">Abrir Simulação →</button>
    </div>

    <div class="card" style="margin-bottom:10px;">
      <div class="ctitle">Resumo por grupo de material planejado (kg)</div>
      <div id="mp-grupo-resumo" class="mp-summary-grid"></div>
    </div>



    <div class="card">
      <div class="ctitle">Necessidade de matéria-prima planejada por semana <span style="font-size:10px;background:#f1efe8;color:#888780;border-radius:4px;padding:1px 7px;margin-left:6px;" id="tcnt-mp">—</span></div>
      <div class="twrap">
        <table>
          <thead id="thead-mp"></thead>
          <tbody id="tbody-mp"></tbody>
        </table>
      </div>
      <div class="pag"><span id="pinf-mp"></span><button id="bpv-mp" onclick="gpMp(-1)">&#8249;</button><button id="bnt-mp" onclick="gpMp(1)">&#8250;</button></div>
    </div>

    <div class="card" id="mp-detail-card" style="display:none;margin-top:10px;">
      <div class="ctitle">Detalhe por peça — <span id="mp-detail-title"></span></div>
      <div class="twrap">
        <table>
          <thead><tr>
            <th>Semana</th><th>Código</th><th>Seq</th><th>Cliente</th><th>Peça</th><th>Liga</th><th>Kg/T</th><th>Qtd. kg</th><th>Metal kg</th>
          </tr></thead>
          <tbody id="tbody-mp-detail"></tbody>
        </table>
      </div>
    </div>
  </div>
  ...
  <!-- ABA SIMULAÇÃO MP -->
  <div class="tab-content" id="tab-simmp">
    <div class="card" style="margin-bottom:10px;display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap;">
      <div>
        <div class="ctitle" style="margin-bottom:3px;">Simulação para compra de matéria-prima</div>
        <div style="font-size:12px;color:#5f5e5a;">Monte cenários com ligas, semanas e quantidades para calcular a necessidade de compra.</div>
      </div>
      <button class="btn" onclick="switchTab('materia')">← Voltar para Matéria Prima</button>
    </div>
    <div class="card" id="simulacao-mp" style="margin-bottom:10px;">
      <div class="ctitle">Simulação para compra de matéria-prima</div>
      <div class="sim-box">
        <div class="sim-form">
          <div><label>Liga</label><select id="sim-liga"><option value="">Escolha a liga</option></select></div>
          <div><label>Semana</label><input id="sim-semana" type="number" placeholder="202626"></div>
          <div><label>Qtd. metal kg</label><input id="sim-qtd" type="number" placeholder="1000"></div>
          <button class="btn" onclick="addSimItem()">Adicionar</button>
          <button class="btn" onclick="clearSim()">Limpar</button>
        </div>
        <div class="sim-form" style="margin-top:8px;">
          <div><label>Adicionar semana planejada</label><select id="sim-semana-planejada"><option value="">Escolha a semana</option></select></div>
          <button class="btn" onclick="addSemanaPlanejada()">Adicionar semana inteira</button>
        </div>
        <div class="sim-mini">Adicione uma liga manualmente ou carregue uma semana inteira planejada. A simulação usa a mesma Receita Padrão.</div>
      </div>
      <div class="ctitle">Itens simulados <span style="font-size:10px;background:#f1efe8;color:#888780;border-radius:4px;padding:1px 7px;margin-left:6px;" id="sim-count">0</span></div>
      <div class="twrap" style="margin-bottom:10px;">
        <table>
          <thead><tr><th>Liga</th><th>Semana</th><th>Origem</th><th style="text-align:right;">Qtd. metal kg</th><th></th></tr></thead>
          <tbody id="tbody-sim-itens"></tbody>
        </table>
      </div>
      <div class="ctitle">Resultado da simulação por matéria-prima</div>
      <div class="twrap">
        <table>
          <thead id="thead-sim"></thead>
          <tbody id="tbody-sim"></tbody>
        </table>
      </div>
    </div>
  </div>
  <!-- Fim ABA SIMULAÇÃO MP -->

  <!-- Fim ABA MATÉRIA PRIMA -->
...
const MP=DATA.mp_records||[];
const RECEITAS=DATA.receitas_records||[];
let simItens=[];
const SEMANA_ATUAL=Number(DATA.semana_atual||0);
let gerFil=[...GER], tabFil=[...TAB], acompFil=[...ACOMP], mpFil=[...MP];
let sk='os', sd=1, p=1, pAcomp=1, pMp=1, skA='os', sdA=1, skMp='total', sdMp=1;
...
const totalLabelPlugin={{id:'totalLabelPlugin',afterDatasetsDraw(chart){{
  ...
  chart.data.labels.forEach((label,i)=>{{
    ...
    if(total>0)ctx.fillText(total.toLocaleString('pt-BR',{{maximumFractionDigits:0}}),x.getPixelForValue(i),y.getPixelForValue(total)-6);
  }});
  ctx.restore();
}}}};
...
function popF(){{
  ...
  // Filtros Matéria Prima
  const addMp=(id,vals)=>{{
    const el=document.getElementById(id);
    [...new Set(vals.filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b))).forEach(v=>{{
      const o=document.createElement('option');
      o.value=v;
      o.textContent=v;
      el.appendChild(o);
    }});
    el.addEventListener('change',applyFMp);
  }};
  addMp('f-mp-ano',MP.map(r=>r.ano));
  addMp('f-mp-semana',MP.map(r=>r.semana).filter(Boolean).map(String));
  addMp('f-mp-liga',MP.map(r=>r.liga));
  addMp('f-mp-desc',MP.map(r=>r.descricao));
  document.getElementById('f-search-mp').addEventListener('input',applyFMp);

  const simLiga=document.getElementById('sim-liga');
  if(simLiga){{
    const ligasReceita=[...new Set(RECEITAS.map(r=>r.liga).filter(Boolean))].sort();
    ligasReceita.forEach(v=>{{const o=document.createElement('option');o.value=v;o.textContent=v;simLiga.appendChild(o);}});
  }}
  const simSemana=document.getElementById('sim-semana-planejada');
  if(simSemana){{
    const semanasPlan=[...new Set(GER.filter(r=>!r.fundido&&r.semana).map(r=>Number(r.semana)))].sort((a,b)=>a-b);
    semanasPlan.forEach(v=>{{const o=document.createElement('option');o.value=v;o.textContent=v;simSemana.appendChild(o);}});
  }}
  carregarSimulacao();
}}
...
function renderT(){{
  ...
  document.getElementById('tbody').innerHTML=sl.map(r=>`<tr><td><strong>${{r.os}}</strong></td><td style="color:#888780">${{r.item}}</td><td style="font-size:11px;color:#888780;">${{r.codigo||''}}</td><td style="font-size:11px;color:#888780;">${{r.sequencia||''}}</td><td>${{r.cliente}}</td><td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${{r.descricao}}">${{r.descricao}}</td><td style="font-size:11px;color:#888780;">${{r.material}}</td><td style="text-align:center;font-weight:500;">${{r.semana||'-'}}</td><td style="font-size:11px;">${{r.prz_fund}}</td><td style="font-size:11px;">${{r.prz_cont}}</td><td style="font-size:11px;color:#5f5e5a;">${{r.etapa}}</td><td style="text-align:center"><button onclick="abrirObservacoes('${{r.chave}}','${{r.codigo||''}}','${{r.sequencia||''}}','geral')" style="background:none;border:none;cursor:pointer;font-size:16px;padding:4px 8px" title="Ver/Adicionar Observações">💬</button></td></tr>`).join('');
...
function renderKpisAcomp(){{
  const noPrazo=acompFil.filter(r=>normStatusAcomp(r.status)==='no prazo').length;
  const potencial=acompFil.filter(r=>{{
    const st=normStatusAcomp(r.status);
    return st==='potencial atraso'||st==='pot. atraso';
  }}).length;
  const atrasado=acompFil.filter(r=>normStatusAcomp(r.status)==='atrasado').length;
  document.getElementById('k-acomp-prazo').textContent=noPrazo.toLocaleString('pt-BR');
  document.getElementById('k-acomp-potencial').textContent=potencial.toLocaleString('pt-BR');
  document.getElementById('k-acomp-atrasado').textContent=atrasado.toLocaleString('pt-BR');
}}
function renderTAcomp(){{
  ...
  document.getElementById('tbody-acomp').innerHTML=sl.map(r=>`<tr>
    <td style="font-size:11px;color:#888780;">${{r.codigo||''}}</td><td style="font-size:11px;color:#888780;">${{r.sequencia||''}}</td><td>${{r.cliente}}</td>
    ...
    <td style="text-align:center"><button onclick="abrirObservacoes('${{r.chave}}','${{r.codigo||''}}','${{r.sequencia||''}}','acomp')" style="background:none;border:none;cursor:pointer;font-size:16px;padding:4px 8px" title="Ver/Adicionar Observações">💬</button></td>
  </tr>`).join('');
...
function applyFMp(){{
  const an=document.getElementById('f-mp-ano').value;
  const sm=document.getElementById('f-mp-semana').value;
  const li=document.getElementById('f-mp-liga').value;
  const de=document.getElementById('f-mp-desc').value;
  const q=document.getElementById('f-search-mp').value.toLowerCase();
  mpFil=MP.filter(r=>{{
    if(an&&r.ano!==an)return false;
    if(sm&&String(r.semana)!==sm)return false;
    if(li&&r.liga!==li)return false;
    if(de&&r.descricao!==de)return false;
    if(q&&!(`${{r.semana}} ${{r.liga}} ${{r.liga_planejada||''}} ${{r.cod}} ${{r.descricao}} ${{r.os||''}} ${{r.item||''}} ${{r.cliente||''}} ${{r.peca||''}}`).toLowerCase().includes(q))return false;
    return true;
  }});
  pMp=1;
  renderKMp();
  renderTMp();
  document.getElementById('mp-detail-card').style.display='none';
}}
function resetFMp(){{
  ['f-mp-ano','f-mp-semana','f-mp-liga','f-mp-desc'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('f-search-mp').value='';
  applyFMp();
}}
function renderKMp(){{
  const totalKg=mpFil.reduce((s,r)=>s+(Number(r.qtd_kg)||0),0);
  const ligas=new Set(mpFil.map(r=>r.liga).filter(Boolean)).size;
  const itens=new Set(mpFil.map(r=>r.chave).filter(Boolean)).size;
  document.getElementById('k-mp-total').textContent=`${{Math.round(totalKg/1000).toLocaleString('pt-BR')}} t`;
  document.getElementById('k-mp-ligas').textContent=ligas.toLocaleString('pt-BR');
  document.getElementById('k-mp-itens').textContent=itens.toLocaleString('pt-BR');
  document.getElementById('mp-label').textContent=`${{mpFil.length}} linhas detalhadas`;
  renderMpGrupoResumo();
}}

function renderMpGrupoResumo(){{
  const el=document.getElementById('mp-grupo-resumo');
  if(!el)return;
  const unicos={{}};
  mpFil.forEach(r=>{{
    const chave=`${{r.chave||''}}|${{r.grupo||''}}|${{r.desc_grupo||''}}`;
    if(!chave.trim())return;
    if(!unicos[chave]){{
      unicos[chave]={{grupo:r.grupo||'',desc_grupo:r.desc_grupo||'Sem grupo',metal:Number(r.metal_planejado_kg)||0}};
    }}
  }});
  const mapa={{}};
  Object.values(unicos).forEach(r=>{{
    const key=`${{r.grupo||''}} - ${{r.desc_grupo||'Sem grupo'}}`;
    if(!mapa[key])mapa[key]={{grupo:r.grupo||'',desc:r.desc_grupo||'Sem grupo',metal:0}};
    mapa[key].metal+=Number(r.metal)||0;
  }});
  const rows=Object.values(mapa).sort((a,b)=>{{
    const na=parseInt(String(a.grupo).replace(/\\D/g,''))||999;
    const nb=parseInt(String(b.grupo).replace(/\\D/g,''))||999;
    if(na!==nb)return na-nb;
    return String(a.desc).localeCompare(String(b.desc));
  }});
  if(!rows.length){{
    el.innerHTML='<div class="mp-summary-item">Sem dados para os filtros selecionados</div>';
    return;
  }}
  el.innerHTML=rows.map(r=>`<div class="mp-summary-item">${{r.desc}} = <span class="mp-summary-value">${{Math.round(r.metal).toLocaleString('pt-BR')}}</span></div>`).join('');
}}
function buildMpMatrix(){{
  const semanas=[...new Set(mpFil.map(r=>Number(r.semana)||0).filter(Boolean))].sort((a,b)=>a-b);
  const mapa={{}};
  mpFil.forEach(r=>{{
    const desc=r.descricao||'Sem descrição';
    if(!mapa[desc])mapa[desc]={{descricao:desc,cods:new Set(),semanas:{{}},total:0,itens:new Set()}};
    const kg=Number(r.qtd_kg)||0;
    const sem=Number(r.semana)||0;
    mapa[desc].cods.add(r.cod||'');
    mapa[desc].semanas[sem]=(mapa[desc].semanas[sem]||0)+kg;
    mapa[desc].total+=kg;
    if(r.chave)mapa[desc].itens.add(r.chave);
  }});
  let rows=Object.values(mapa).map(r=>({{descricao:r.descricao,cod:[...r.cods].filter(Boolean).join(', '),semanas:r.semanas,total:r.total,itens:r.itens.size}}));
  rows.sort((a,b)=>{{
    if(skMp==='descricao')return a.descricao.localeCompare(b.descricao)*sdMp;
    if(skMp==='cod')return a.cod.localeCompare(b.cod)*sdMp;
    if(skMp==='itens')return (a.itens-b.itens)*sdMp;
    if(String(skMp).startsWith('sem_')){{
      const sem=String(skMp).replace('sem_','');
      return ((a.semanas[sem]||0)-(b.semanas[sem]||0))*sdMp;
    }}
    return (a.total-b.total)*sdMp;
  }});
  return {{semanas,rows}};
}}
function srtMp(k){{if(skMp===k)sdMp*=-1;else{{skMp=k;sdMp=1;}}renderTMp();}}
function renderTMp(){{
  const {{semanas,rows}}=buildMpMatrix();
  document.getElementById('thead-mp').innerHTML=`<tr>
    <th onclick="srtMp('descricao')">Descrição matéria-prima</th>
    ${{semanas.map(s=>`<th onclick="srtMp('sem_${{s}}')" style="text-align:right;">${{s}}</th>`).join('')}}
    <th onclick="srtMp('total')" style="text-align:right;">Total kg</th>
    <th onclick="srtMp('itens')" style="text-align:right;">Itens</th>
  </tr>`;
  const sl=rows.slice((pMp-1)*PP,pMp*PP);
  document.getElementById('tbody-mp').innerHTML=sl.map(r=>`<tr>
    <td><span class="mp-link" onclick="showMpDetail('${{encodeURIComponent(r.descricao)}}')">${{r.descricao}}</span></td>
    ${{semanas.map(s=>`<td style="text-align:right;">${{Math.round(r.semanas[s]||0).toLocaleString('pt-BR')}}</td>`).join('')}}
    <td style="text-align:right;font-weight:700;">${{Math.round(r.total||0).toLocaleString('pt-BR')}}</td>
    <td style="text-align:right;">${{(Number(r.itens)||0).toLocaleString('pt-BR')}}</td>
  </tr>`).join('');
  const pages=Math.max(1,Math.ceil(rows.length/PP));
  document.getElementById('tcnt-mp').textContent=rows.length;
  document.getElementById('pinf-mp').textContent=`Pág. ${{pMp}} de ${{pages}}`;
  document.getElementById('bpv-mp').disabled=pMp<=1;
  document.getElementById('bnt-mp').disabled=pMp>=pages;
}}
function gpMp(d){{pMp+=d;renderTMp();}}
function showMpDetail(descEncoded){{
  const desc=decodeURIComponent(descEncoded);
  const rows=mpFil.filter(r=>r.descricao===desc).sort((a,b)=>(Number(a.semana)||0)-(Number(b.semana)||0)||String(a.os).localeCompare(String(b.os)));
  const total=rows.reduce((s,r)=>s+(Number(r.qtd_kg)||0),0);
  document.getElementById('mp-detail-card').style.display='block';
  document.getElementById('mp-detail-title').textContent=`${{desc}} · ${{Math.round(total).toLocaleString('pt-BR')}} kg · ${{new Set(rows.map(r=>r.chave)).size}} itens`;
  document.getElementById('tbody-mp-detail').innerHTML=rows.map(r=>`<tr>
    <td style="text-align:center;font-weight:600;">${{r.semana||'-'}}</td>
    <td style="font-size:11px;color:#888780;">${{r.codigo||''}}</td>
    <td style="font-size:11px;color:#888780;">${{r.sequencia||''}}</td>
    <td>${{r.cliente||''}}</td>
    <td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${{r.peca||''}}">${{r.peca||''}}</td>
    <td>${{r.liga_planejada||r.liga||''}}</td>
    <td style="text-align:right;">${{Number(r.kg_por_tonelada||0).toLocaleString('pt-BR')}}</td>
    <td style="text-align:right;font-weight:600;">${{Math.round(Number(r.qtd_kg)||0).toLocaleString('pt-BR')}}</td>
    <td style="text-align:right;">${{Math.round(Number(r.metal_planejado_kg)||0).toLocaleString('pt-BR')}}</td>
  </tr>`).join('');
  document.getElementById('mp-detail-card').scrollIntoView({{behavior:'smooth',block:'start'}});
}}

function normalizarLigaJS(v){{
  return String(v||'').toUpperCase().replace(/[^A-Z0-9]/g,'');
}}
function receitasDaLiga(liga){{
  const key=normalizarLigaJS(liga);
  return RECEITAS.filter(r=>normalizarLigaJS(r.liga)===key || String(r.liga_key||'')===key);
}}
function carregarSimulacao(){{
  try{{simItens=JSON.parse(localStorage.getItem('simMateriaPrima')||'[]')||[];}}catch(e){{simItens=[];}}
  renderSimulacao();
}}
function salvarSimulacao(){{
  try{{localStorage.setItem('simMateriaPrima',JSON.stringify(simItens));}}catch(e){{}}
}}
function goSimulacao(){{
  switchTab('simmp');
  window.scrollTo({{top:0,behavior:'smooth'}});
}}
function addSemanaPlanejada(){{
  const semana=Number(document.getElementById('sim-semana-planejada').value)||0;
  if(!semana){{alert('Escolha uma semana planejada.');return;}}
  const itens=GER.filter(r=>!r.fundido && Number(r.semana)===semana);
  if(!itens.length){{alert('Não encontrei itens planejados nessa semana.');return;}}
  const porLiga={{}};
  let ignorados=0;
  itens.forEach(r=>{{
    const liga=String(r.liga_planejada||r.material||'').trim();
    const qtd=Number(r.metal_liq_calc)||0;
    if(!liga||qtd<=0){{ignorados++;return;}}
    if(!receitasDaLiga(liga).length){{ignorados++;return;}}
    porLiga[liga]=(porLiga[liga]||0)+qtd;
  }});
  const entradas=Object.entries(porLiga);
  if(!entradas.length){{alert('Nenhuma liga dessa semana possui receita padrão cadastrada.');return;}}
  entradas.forEach(([liga,qtd])=>simItens.push({{id:Date.now()+Math.random(),liga,semana,qtd,origem:'Semana planejada'}}));
  salvarSimulacao();
  renderSimulacao();
  alert(`Semana ${{semana}} adicionada: ${{entradas.length}} ligas. ${{ignorados?ignorados+' itens ignorados por falta de liga/receita.':''}}`);
}}
function addSimItem(){{
  const liga=document.getElementById('sim-liga').value;
  const semana=Number(document.getElementById('sim-semana').value)||0;
  const qtd=Number(document.getElementById('sim-qtd').value)||0;
  if(!liga){{alert('Escolha uma liga.');return;}}
  if(!semana){{alert('Informe a semana no formato AAAASS, exemplo 202626.');return;}}
  if(qtd<=0){{alert('Informe a quantidade de metal em kg.');return;}}
  if(!receitasDaLiga(liga).length){{alert('Não encontrei receita para essa liga.');return;}}
  simItens.push({{id:Date.now()+Math.random(),liga,semana,qtd}});
  document.getElementById('sim-qtd').value='';
  salvarSimulacao();
  renderSimulacao();
}}
function removeSimItem(id){{
  simItens=simItens.filter(x=>String(x.id)!==String(id));
  salvarSimulacao();
  renderSimulacao();
}}
function clearSim(){{
  if(simItens.length && !confirm('Limpar todos os itens da simulação?'))return;
  simItens=[];
  salvarSimulacao();
  renderSimulacao();
}}
function simDetalhes(){{
  const out=[];
  simItens.forEach((it,idx)=>{{
    receitasDaLiga(it.liga).forEach(rec=>{{
      const qtd=(Number(it.qtd)||0)/1000*(Number(rec.kg_por_tonelada)||0);
      out.push({{idx,liga:it.liga,semana:Number(it.semana)||0,metal:Number(it.qtd)||0,cod:rec.cod||'',descricao:rec.descricao||'Sem descrição',kg_por_tonelada:Number(rec.kg_por_tonelada)||0,qtd_kg:qtd}});
    }});
  }});
  return out;
}}
function renderSimulacao(){{
  const tbodyItens=document.getElementById('tbody-sim-itens');
  const count=document.getElementById('sim-count');
  if(!tbodyItens)return;
  count.textContent=simItens.length;
  tbodyItens.innerHTML=simItens.map(it=>`<tr>
    <td>${{it.liga}}</td>
    <td style="text-align:center;font-weight:600;">${{it.semana}}</td>
    <td style="color:#888780;font-size:11px;">${{it.origem||'Manual'}}</td>
    <td style="text-align:right;">${{Math.round(Number(it.qtd)||0).toLocaleString('pt-BR')}}</td>
    <td style="text-align:right;"><button class="btn" onclick="removeSimItem('${{it.id}}')">Remover</button></td>
  </tr>`).join('') || '<tr><td colspan="5" style="color:#888780;">Nenhum item simulado ainda.</td></tr>';

  const detalhes=simDetalhes();
  const semanas=[...new Set(detalhes.map(r=>r.semana).filter(Boolean))].sort((a,b)=>a-b);
  const mapa={{}};
  detalhes.forEach(r=>{{
    const d=r.descricao||'Sem descrição';
    if(!mapa[d])mapa[d]={{descricao:d,semanas:{{}},total:0}};
    mapa[d].semanas[r.semana]=(mapa[d].semanas[r.semana]||0)+(Number(r.qtd_kg)||0);
    mapa[d].total+=Number(r.qtd_kg)||0;
  }});
  const rows=Object.values(mapa).sort((a,b)=>b.total-a.total);
  document.getElementById('thead-sim').innerHTML=`<tr><th>Descrição matéria-prima</th>${{semanas.map(s=>`<th style="text-align:right;">${{s}}</th>`).join('')}}<th style="text-align:right;">Total kg</th></tr>`;
  document.getElementById('tbody-sim').innerHTML=rows.map(r=>`<tr>
    <td>${{r.descricao}}</td>
    ${{semanas.map(s=>`<td style="text-align:right;">${{Math.round(r.semanas[s]||0).toLocaleString('pt-BR')}}</td>`).join('')}}
    <td style="text-align:right;font-weight:700;">${{Math.round(r.total||0).toLocaleString('pt-BR')}}</td>
  </tr>`).join('') || '<tr><td colspan="2" style="color:#888780;">Adicione ligas para simular a compra de matéria-prima.</td></tr>';
}}
...
popF();applyF();applyFMp();

// ═══════════════════════════════════════════════════════════════════════════
// SISTEMA DE OBSERVAÇÕES - GOOGLE APPS SCRIPT
// ═══════════════════════════════════════════════════════════════════════════
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzLZistVHMvdnOGlGRcKXHurBfy-4vBKjyiksLruyLyTOiSfMdKI8PGgQSrtIRnkBWFNg/exec';

let observacoesCache = {{}};
let observacoesCarregadas = false;

// Carregar observações do Google Sheets via Apps Script
async function carregarObservacoes() {{
  try {{
    ...
    result.data.forEach(row => {{
      const [timestamp, chave, codigo, sequencia, usuario, observacao] = row;
      if (!chave) return;
      
      if (!observacoesCache[chave]) {{
        observacoesCache[chave] = [];
      }}
      
      observacoesCache[chave].push({{
        timestamp,
        codigo,
        sequencia,
        usuario,
    ...
    observacoesCarregadas = true;
  }} catch (err) {{
    console.error('Erro ao carregar observações:', err);
  }}
}}

// Salvar observação no Google Sheets via Apps Script
async function salvarObservacao(chave, codigo, sequencia, observacao) {{
  try {{
    const timestamp = new Date().toLocaleString('pt-BR');
    const usuario = 'João'; // Pode melhorar depois para pegar usuário do Windows
    ...
    // Usar fetch sem no-cors para permitir verificação de resposta
    const response = await fetch(APPS_SCRIPT_URL, {{
      method: 'POST',
...
async function abrirObservacoes(chave, codigo, sequencia, aba) {{
  // Se ainda não carregou as observações, carregar agora
  if (!observacoesCarregadas) {{
    ...
    loading.innerHTML = '<div style="background:white;padding:20px;border-radius:8px;color:#5f5e5a">⏳ Carregando observações...</div>';
  ...
  const obs = observacoesCache[chave] || [];
  
  const modal = document.createElement('div');
  modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:9999';
  
  const box = document.createElement('div');
  box.style.cssText = 'background:white;border-radius:8px;padding:24px;width:90%;max-width:600px;max-height:80vh;overflow:auto;box-shadow:0 4px 20px rgba(0,0,0,0.3)';
  
  let html = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;border-bottom:2px solid #f1efe8;padding-bottom:12px">
      <h3 style="margin:0;color:#2c2b28">💬 Observações - ${{codigo||chave}} ${{sequencia?'('+sequencia+')':''}}</h3>
      <button onclick="this.closest('[style*=fixed]').remove()" style="background:none;border:none;font-size:24px;cursor:pointer;color:#888780">&times;</button>
    </div>
  `;
  
  // Histórico
  if (obs.length > 0) {{
    html += '<div style="margin-bottom:20px"><h4 style="color:#5f5e5a;font-size:12px;text-transform:uppercase;margin-bottom:8px">Histórico</h4>';
    obs.slice().reverse().forEach(o => {{
      html += `
        <div style="background:#f9f8f6;padding:12px;border-radius:6px;margin-bottom:8px;border-left:3px solid #185fa5">
          <div style="display:flex;justify-content:space-between;margin-bottom:4px">
            <strong style="color:#2c2b28;font-size:13px">${{o.usuario}}</strong>
            <span style="color:#888780;font-size:11px">${{o.timestamp}}</span>
          </div>
          <p style="margin:0;color:#5f5e5a;font-size:13px">${{o.observacao}}</p>
        </div>
      `;
    }});
    html += '</div>';
  }} else {{
    html += '<p style="color:#888780;font-style:italic;margin-bottom:20px">Nenhuma observação ainda.</p>';
  }}
  
  // Campo para nova observação
  html += `
    <div>
      <h4 style="color:#5f5e5a;font-size:12px;text-transform:uppercase;margin-bottom:8px">Nova Observação</h4>
      <textarea id="nova-obs" style="width:100%;min-height:80px;padding:12px;border:1px solid #ddd;border-radius:6px;font-family:inherit;font-size:13px;resize:vertical" placeholder="Digite sua observação..."></textarea>
      <div style="display:flex;gap:8px;margin-top:12px">
        <button onclick="salvarNovaObs('${{chave}}','${{codigo||''}}','${{sequencia||''}}')" style="flex:1;background:#185fa5;color:white;border:none;padding:10px;border-radius:6px;font-weight:600;cursor:pointer;font-size:13px">💾 Salvar</button>
        <button onclick="this.closest('[style*=fixed]').remove()" style="flex:1;background:#e8e7e4;color:#5f5e5a;border:none;padding:10px;border-radius:6px;font-weight:600;cursor:pointer;font-size:13px">Cancelar</button>
      </div>
    </div>
  `;
  ...
  // Focar no textarea
  setTimeout(() => document.getElementById('nova-obs').focus(), 100);
}}

// Salvar nova observação
async function salvarNovaObs(chave, codigo, sequencia) {{
  const texto = document.getElementById('nova-obs').value.trim();
  if (!texto) {{
    alert('Digite uma observação primeiro!');
    return;
  }}
  
  const sucesso = await salvarObservacao(chave, codigo, sequencia, texto);
  if (sucesso) {{
    // Fechar modal
...
// Carregar observações ao iniciar (em background, sem bloquear)
carregarObservacoes();
</script>
</body>
</html>"""
...
if __name__ == '__main__':
    ...
    try:
        ...
        receitas_padrao = data.get('receitas_padrao')
        if resp == 's':
            salvar_excel(acomp_original, ger_processado, fusao, receitas_padrao)

        log('\nGerando dashboard HTML...')
        data.pop('ger_processado', None)
        data.pop('acomp_original', None)
        data.pop('receitas_padrao', None)
        gerar_html(data)

        # ========== GIT PUSH AUTOMÁTICO ==========
        resp_git = perguntar('\nEnviar para GitHub? (s/n) [s]: ', 's')
        if resp_git == 's':
            ...
            try:
                ...
                mensagem = f"Atualizar dashboard - {datetime.now().strftime('%d/%m/%Y %H:%M')}"
                subprocess.run(['git', 'commit', '-m', mensagem], check=True, capture_output=True)
                subprocess.run(['git', 'push'], check=True, capture_output=True)
                
                log('✅ Enviado para GitHub com sucesso!')
                log('🌐 Aguarde 1 minuto e atualize a página (Ctrl+F5)')
            except subprocess.CalledProcessError:
                log('⚠️  Git não configurado (normal se não usa GitHub Pages)')
            except FileNotFoundError:
                log('⚠️  Git não encontrado (instale em: https://git-scm.com)')
            except Exception as e:
